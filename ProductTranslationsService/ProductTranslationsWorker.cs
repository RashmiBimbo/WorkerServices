...
{
    public class ProductTranslationsWorker : BackgroundService
    {
        private int _executionCount;
        private ProductTranslationsContext cntxt;
        private string msg;
        private readonly string logFile;
        double period = 30;
        private readonly IServiceScopeFactory serviceScopeFactory;
        private readonly ILogger<ProductTranslationsWorker> logger;
        private ProductTranslationsBase Poco;

        public ProductTranslationsWorker(IServiceScopeFactory serviceScopeFactory, ILogger<ProductTranslationsWorker> logger)
        {
            this.serviceScopeFactory = serviceScopeFactory;
            this.logger = logger;
            //Poco = cust;
            logFile = AppDomain.CurrentDomain.BaseDirectory + "ProductTranslationsService_Log.txt";
            try
            {
        ...
        private async Task JsonToDb(string result)
        {
            ...
            try
            {
                ...
                cntxt = scope.ServiceProvider.GetRequiredService<ProductTranslationsContext>();
                ...
                await strategy.ExecuteAsync(async () =>
                {
                    using (var transaction = await cntxt.Database.BeginTransactionAsync())
                    {
                        ...
                        {
                            string itmJsn;
                            ProductTranslationsTestR existingEntity = null;
                            for (int cnt = 1; cnt <= 2; cnt++)
                            {
                                try
                                {
                                    itmJsn = Serialize.ToJson(itm);
                                    ProductTranslationsTestR poco = JsonConvert.DeserializeObject<ProductTranslationsTestR>(itmJsn);
                                    if (poco is null) continue;

                                    // Find existing entity in the database
                                    if (cntxt.ProductTranslationsTestR.Any())
                                        existingEntity = await cntxt.ProductTranslationsTestR.AsNoTracking().FirstOrDefaultAsync(e => e.ProductNumber == poco.ProductNumber);

                                    if (!cntxt.ProductTranslationsTestR.Local.Any(e => e.ProductNumber == poco.ProductNumber && e.LanguageId == poco.LanguageId))
                                    {
                                        // Check if the entity exists in the database
                                        if (existingEntity == null) // Add the new entity
                                        {
                                            cntxt.ProductTranslationsTestR.Add(poco);
                                            addCnt++;
                                        }
        ...
        private async Task<bool> CheckTableExists(ProductTranslationsContext cntxt)
        {
            for (int i = 0; i < 3; i++)
            {
                ...
                try
                {
                    ...
                    var testQuery = await cntxt.ProductTranslationsTestR.FirstOrDefaultAsync();
                    break;
                }
                catch (Exception ex1)
                {
        ...
        private async Task ApplyMigration(ProductTranslationsContext cntxt)
        {
            try
            {
