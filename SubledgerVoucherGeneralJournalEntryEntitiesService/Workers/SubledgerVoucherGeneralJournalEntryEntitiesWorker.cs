                        ...
                        foreach (var itm in Items)
                        {
                            ...
                            for (int cnt = 1; cnt <= 2; cnt++)
                            {
                                try
                                {
                                    ...
                                    if (cntxt.SubledgerVoucherGeneralJournalEntryEntitiesTestR.Any())
                                    ...
                                    if (cntxt.SubledgerVoucherGeneralJournalEntryEntitiesTestR.Local.Count > 0 && !cntxt.SubledgerVoucherGeneralJournalEntryEntitiesTestR.Local.Any(e => e.Voucher == poco.Voucher && e.RecId1 == poco.RecId1))
                                    {
        ...
        private async Task<bool> CheckTableExists(ServiceDbContext cntxt)
        {
            for (int i = 0; i < 3; i++)
            {
                ...
                try
                {
                    ...
                    var testQuery = await cntxt.SubledgerVoucherGeneralJournalEntryEntitiesTestR.FirstAsync();
                    break;
                }
                catch (Exception ex1)
                {
