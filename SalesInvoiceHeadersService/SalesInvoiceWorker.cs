    ...
    public class SalesInvoiceWorker : BackgroundService
    {
        ...
        private async Task JsonToDb(string result)
        {
            ...
            try
            {
                ...
                {
                    string itmJsn;
                    CustInvoiceJourTestR? existingEntity;
                    for (int cnt = 1; cnt <= 2; cnt++)
                    {
                        try
                        {
                            ...
                            //existingEntity = await cntxt.CustInvoiceJourTestR.AsNoTracking().FirstOrDefaultAsync(e => e.Etag == poco.Etag);
                            existingEntity = await cntxt.CustInvoiceJourTestR.FindAsync(poco.RecId1);

                            // Check if the entity exists in the database
                            if (existingEntity == null) // Add the new entity
                            {
                                cntxt.CustInvoiceJourTestR.Add(poco);
                                addCnt++;
                            }
