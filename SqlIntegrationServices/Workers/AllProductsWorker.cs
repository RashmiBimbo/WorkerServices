...
{
    //internal class AllProductsWorker(IServiceScopeFactory serviceScopeFactory, ILogger<BaseWorker> logger, ServiceDetails config) : BaseWorker(serviceScopeFactory, logger, config)
    //{
    ...
    //        {
    //            return await UpdateTbl<SqlIntegrationServices.AllProductsTest>(Items);
    //        }
    //        else
    //        {
    //            return await UpdateTbl<SqlIntegrationServices.AllProducts>(Items);
    //        }
    //    }
    //}
    ...
    //        try
    //        {
    //            LogInfo($"\r\n{Now}: All Products ServiceDetails running.");
    ...
    //                string msg = $"{Now}: All Products ServiceDetails is working; Count: {count}";
    //                LogInfo(msg);
    ...
    //        finally
    //        {
    //            LogInfo($"{Now}: All Products ServiceDetails stopped.");
    //        }
    //    }
    //    //, stoppingToken);
    //    //    return Task.CompletedTask;
    ...
    //                    {
    //                        string itmJsn;
    //                        AllProductsTest existingEntity = null;
    //                        for (int cnt = 1; cnt <= 2; cnt++)
    //                        {
    //                            try
    //                            {
    //                                itmJsn = itm.ToJson();
    //                                AllProductsTest poco = JsonConvert.DeserializeObject<AllProductsTest>(itmJsn);
    //                                if (poco is null) continue;

    //                                // Find existing entity in the database
    //                                if (cntxt.AllProductsTest.Count() > 0)
    //                                    existingEntity = await cntxt.AllProductsTest.AsNoTracking().FirstOrDefaultAsync(e => e.ProductNumber == poco.ProductNumber);

    //                                if (!cntxt.AllProductsTest.Local.Any(e => e.ProductNumber == poco.ProductNumber))
    //                                {
    //                                    // Check if the entity exists in the database
    //                                    if (existingEntity == null) // Add the new entity
    //                                    {
    //                                        cntxt.AllProductsTest.Add(poco);
    //                                        addCnt++;
    //                                    }
    ...
    //                var testQuery = await cntxt.AllProductsTest.FirstOrDefaultAsync();
    //                break;
    //            }
    //            catch (Exception ex1)
    //            {
