...
namespace SqlIntegrationServices.Workers
{
    public class UnitOfMeasureConversionsWorker : BackgroundService
    {
                ...
                foreach (var itm in Items)
                {
                    ...
                    for (int cnt = 1; cnt <= 2; cnt++)
                    {
                        try
                        {
                            ...
                            {
                                UnitOfMeasureConversionsTestR ent = PrepareEntity(poco);

                                if (existingEntity == null) // Add the new entity
                                {
                                    cntxt.UnitOfMeasureConversionsTestR.Add(ent);
                                    addCnt++;
                                }
        ...
        private UnitOfMeasureConversionsTestR PrepareEntity(UnitOfMeasureConversionsTestR poco)
        {
            UnitOfMeasureConversionsTestR ent = new();
            try
            {
                foreach (var col in config.Columns)
                {
                    string propName = col.Name;
                    var propInfo = typeof(UnitOfMeasureConversionsTestR).GetProperty(propName);
        ...
        private async Task<bool> CheckTableExists(ServiceDbContext cntxt)
        {
            for (int i = 0; i < 3; i++)
            {
                try
                {
                    ...
                    var testQuery = await cntxt.UnitOfMeasureConversionsTestR.FirstOrDefaultAsync();
                    break;
                }
                catch (Exception ex1)
                {
