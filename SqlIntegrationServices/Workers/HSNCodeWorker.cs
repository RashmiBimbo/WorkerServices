...
{
    internal class HSNCodesWorker(IServiceScopeFactory serviceScopeFactory, ILogger<BaseWorker> logger, Service config) : BaseWorker(serviceScopeFactory, logger, config)
    {
        protected override async Task<List<long>> JsonToDbChild(JArray Items, string tableName)
        {
            if (config.Table.Contains("TestR", StringComparison.OrdinalIgnoreCase))
            {
                //if (await CheckTableExists<HSNCodesTestR>())
                    return await UpdateTblHSNCodesTestR<SqlIntegrationServices.HSNCodesTestR>(Items, "HSNCodes");
            }
            else
            {
                //if (await CheckTableExists<HSNCodes>())
                    return await UpdateTblHSNCodes(Items);
            }
        }
